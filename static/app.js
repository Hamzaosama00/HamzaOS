...
let groups=['Morning','Afternoon','Evening','Night'];let icons={Morning:'☀',Afternoon:'◷',Evening:'◑',Night:'☾'};$('#routineGroups').innerHTML=groups.map(g=>{const list=tasks.filter(t=>t.daypart===g);return list.length?`<div class="group-panel"><div class="group-title"><h3>${icons[g]} &nbsp;${g}</h3><span>${list.filter(t=>t.done).length}/${list.length} DONE</span></div>${list.map(taskMarkup).join('')}</div>`:''}).join('');$('#coachBadge').textContent=ai_enabled?'OLLAMA CONNECTED':'STARTER MODE';$('#chatHeadStatus').textContent=ai_enabled?app.ai_provider+' · '+app.ai_model:'Configure Ollama to start chatting';renderHistory(history);renderProfile(profile);renderSettings(app.ai_settings, plan, done);renderChat(app.messages);renderCoachInsights(app.coach_insights)}
...
function renderCoachInsights(insights){
  if(!insights)return;
  const top=(insights.top_missed_categories||[]).map(x=>`<span>${esc(x.category)} · ${Number(x.count)||0}</span>`).join('');
  $('#coachMode').textContent=insights.mode||'Advanced Coach v3';
  $('#coachGoal').textContent=insights.goal||'Slim-fit home workout system.';
  $('#intelCompletion').textContent=(insights.completion_rate??0)+'%';
  $('#intelFocus').textContent=insights.next_focus||'Build consistency';
  $('#missedCats').innerHTML=top||'<span>No missed pattern yet</span>';
}
...
document.addEventListener('click',e=>{const goto=e.target.closest('[data-goto]');if(goto)navigate(goto.dataset.goto);const nav=e.target.closest('[data-page]');if(nav)navigate(nav.dataset.page);const prompt=e.target.closest('[data-prompt]');if(prompt){$('#chatInput').value=prompt.dataset.prompt;$('#chatForm').requestSubmit();}});
...
$('#smartChallenge')?.addEventListener('click',async()=>{
  const b=$('#smartChallenge'); if(!b||b.disabled)return; b.disabled=true; const old=b.textContent; b.textContent='Generating…';
  try{await api('challenge','POST',{});showToast('Smart challenge generated.');await refresh();navigate('coach')}
